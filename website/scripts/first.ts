...
//scroll with mouse
window.onmousedown = (mouse: MouseEvent) => {
...
window.onmousemove = (mouse: MouseEvent) => {
  if (grid.dataset.mouseDownAt === "0") return;
  const mouseDelta = parseFloat(grid.dataset.mouseDownAt) - mouse.clientX,
    maxDelta = window.innerWidth / 2;
  const percentage = (mouseDelta / maxDelta) * -100,
    nextPercentage = Math.max(
      Math.min(parseFloat(grid.dataset.prevPercentage) + percentage, 0),
      -100
    );
...
document.addEventListener("wheel", function (scroll) {
  ...
  const collection = grid.getElementsByClassName(
    "grid-item"
...
function moveClick(placeHtml: string, newHtml: string) {
...
function moveAbout(section: string) {
